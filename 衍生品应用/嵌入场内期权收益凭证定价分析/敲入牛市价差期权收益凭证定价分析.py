...
def get_option_portfolio_value_from_Wind(option_portfolio, date):
    option_portfoio_b = w.wss(option_portfolio['B'], "close,exe_ratio,exe_price", "tradeDate="+date+";priceAdj=U;cycle=D").Data
    option_portfoio_s = w.wss(option_portfolio['S'], "close,exe_ratio,exe_price", "tradeDate="+date+";priceAdj=U;cycle=D").Data
    ...
    return value, number, lower_price, higher_price


def bull_spread_income_certificate(start_date, end_date, underlying_index, underlying_asset, basic_return_rate, bull_spreads, principle, fixed_return_rate):
    '''
    # 产品基本特征
    :param start_date:
    :param end_date:
    ...
    # 产品理论到期日收益情况
    # 计算价差期权组合单位价值和单位数量
    option_value, option_number, asset_lower, asset_higher = get_option_portfolio_value_from_Wind(bull_spreads, start_date)
    ...
    print('\n挂钩指数：%s，%s日收盘点位：%.2f点（100%%）' % (underlying_index, start_date, underlying_index_start),
          '\n浮动下限：%.2f点（%.2f%%）' % (index_lower, index_lower/underlying_index_start*100),
          '\n浮动上限：%.2f点（%.2f%%）' % (index_higher, index_higher/underlying_index_start*100))
    # 计算最佳情形期权投资的总盈亏
    ...
    option_value, _, _, _ = get_option_portfolio_value_from_Wind(bull_spreads, end_date)
...
if __name__ == "__main__":
    ...
    bull_spread_income_certificate(START_DATE, END_DATE, UNDERLYING_INDEX, UNDERLYING_ASSET, BASIC_RATE_RATE,
